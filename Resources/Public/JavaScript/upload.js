	...
	$(function() {
		...
		uploader.bind('Error', function(up, err) {
			...
			switch (err.code) {
				case -600:
					...
					if (window.Typo3Neos) {
						message = window.Typo3Neos.I18n.translate('fileSizeExceedsAllowedLimit', message, 'Flownative.Media.Browser', 'Main', [readableFileSize, readableMaximumFileSize]);
					}
					break;
				default:
					message = err.message;
			...
			if (err.file) {
				message += ' ';
				if (window.Typo3Neos) {
					message += window.Typo3Neos.I18n.translate('forTheFile', 'for the file', 'Flownative.Media.Browser');
				} else {
		...
		uploader.bind('UploadComplete', function(up, file) {
			if (preventReload) {
				...
				if (window.Typo3Neos) {
					message = window.Typo3Neos.I18n.translate('onlySomeFilesWereUploaded', message, 'Flownative.Media.Browser');
