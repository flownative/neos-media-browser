(function($) {
		...
		$(this).droppable({
			...
			drop: function (event, ui) {
				...
				$.post(
					...
					if (window.Typo3Neos) {
						message = window.Typo3Neos.I18n.translate('taggingAssetsFailed', message, 'Flownative.Media.Browser');
		...
		$(this).droppable({
			...
			drop: function (event, ui) {
				...
				$.post(
					...
					if (window.Typo3Neos) {
						message = window.Typo3Neos.I18n.translate('addingAssetsToCollectionFailed', message, 'Flownative.Media.Browser');
						window.Typo3Neos.Notification.error(message);
					} else {
						alert(message);
					}
	...
	$('.neos-media-aside-group > form').submit(function(e) {
		...
		if ($.trim(value.val()) === '') {
			...
			var label = window.Typo3Neos.I18n.translate('creating', 'Creating', 'Flownative.Media.Browser');
